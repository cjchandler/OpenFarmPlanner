...
# ~ def dump_farm_data_json( f):
    # ~ dtn = datetime.now()
    # ~ filename = f.farm_name + "_farm_state.json"
    # ~ out_file = open(filename, "w")

    # ~ json.dump(f, out_file, indent = 6)

    # ~ out_file.close()

# ~ def load_farm_data_json( f):
    # ~ dtn = datetime.now()
    # ~ filename = f.farm_name + "_farm_state.json"
    # ~ out_file = open(filename, "w")
...
class crop_plan:
    ...
    def dump_to_json(self ,path ):
        ...
        dtn = datetime.now()
        filename = str(dtn) + "_crop_plan.json"
        out_file = open(filename, "w")
