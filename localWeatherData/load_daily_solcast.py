...
def load_daily_climate_change_canada( filelist  ):
        df_yar = pd.DataFrame()
        dfwater = pd.DataFrame()
...
def load_daily_climate_weather_gc( filelist  ):
        df_yar = pd.DataFrame()
        for a in filelist: #["yarA_2024.csv" ]:
            path = "./"
            filestring = a 
            dfwater = pd.read_csv(path + filestring, sep=',')
            print(a)
